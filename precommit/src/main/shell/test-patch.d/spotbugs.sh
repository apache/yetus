...
function spotbugs_precheck
{
  ...
  if [[ ${status} == 1 ]]; then
    add_vote_table 0 "${SPOTBUGS_MODE}" "SpotBugs/FindBugs executables are not available."
...
function spotbugs_runner
{
  ...
  if [[ -n "${SPOTBUGS_TABLE_DUPE}" && "${name}" == branch ]]; then
  ...
  until [[ ${i} -eq ${#MODULE[@]} ]]; do
    ...
    if [[ "${#files[@]}" -lt 1 ]]; then
      module_status ${i} -1 "" "${name}/${module} no SpotBugs/FindBugs output file (${targetfile})"
      ((i=i+1))
      continue
    fi
...
function spotbugs_preapply
{
  ...
  until [[ ${modindex} -eq ${#MODULE[@]} ]]; do
    ...
    if [[ ${module_spotbugs_warnings} -gt 0 ]] ; then
      msg="${module} in ${PATCH_BRANCH} has ${module_spotbugs_warnings} extant SpotBugs/FindBugs warnings."
